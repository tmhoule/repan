  ...
  try {
    ...
    if (allTeams) {
      ...
      const usersWithTeams = await prisma.user.findMany({
        where,
        select: {
          id: true, name: true, role: true, avatarColor: true, isActive: true, isSuperAdmin: true, ssoUser: true, createdAt: true, lastLoginAt: true,
      ...
      return NextResponse.json(usersWithTeams.map((u) => ({
        ...
        isActive: u.isActive, isSuperAdmin: u.isSuperAdmin, ssoUser: u.ssoUser, createdAt: u.createdAt, lastLoginAt: u.lastLoginAt,
